...
let handle_loc (j: Basic.json list) = match j with
  | [`Int a; `Int b] -> (a,b)
  | _ -> failwith "error"

(*[handle_loc] helper function for reading a string from a json file*)
let handle_str (j: Basic.json list) = match j with 
  | [`String a] -> a
  | _ -> failwith "error"

(*[handle_loc] helper function for reading a list from a json file*)
let handle_lst (j: (color_t * Basic.json) list) = match j with
...
let rd_col (j:Basic.json list) : (location_t*particle_t) list = 
...
let read_state (path:file_path_t) : Model.ArrayModel.grid_t = 
  let j = Yojson.Basic.from_file path in
  ...
  Array.fold_left (fun acc_row row -> 
      Array.append acc_row [|(Array.of_list row)|] ) [||] json_lst_arr

(*[wr_row] converts a row of the grid into json*)
let wr_row (arr:((int*int)* particle_t) array) : Basic.json = 
...
let write_state grid:(ArrayModel.grid_t) : file_path_t=  
  (`List (Array.fold_left (fun acc r -> acc@[(wr_row r)]) [] grid) )
  |> to_file "grid.json"