open Lwt
open Lwt_react
open LTerm_geom
open LTerm_text
open LTerm_key
open LTerm_draw

type gui_t = unit

let rec get_inputs ui coord =
  LTerm_ui.wait ui >>= function
    | LTerm_event.Key{ code = Up } ->
        coord := { !coord with row = !coord.row - 1 };
        LTerm_ui.draw ui;
        get_inputs ui coord
    | LTerm_event.Key{ code = Down } ->
        coord := { !coord with row = !coord.row + 1 };
        LTerm_ui.draw ui;
        get_inputs ui coord
    | LTerm_event.Key{ code = Left } ->
        coord := { !coord with col = !coord.col - 1 };
        LTerm_ui.draw ui;
        get_inputs ui coord
    | LTerm_event.Key{ code = Right } ->
        coord := { !coord with col = !coord.col + 1 };
        LTerm_ui.draw ui;
        get_inputs ui coord
    | LTerm_event.Key{ code = Escape } ->
        return ()
    | ev ->
        get_inputs ui coord

(* let get_window_size = failwith "unimplemented"

let draw_to_screen = failwith "unimplemented" *)

let quit_gui ui = fun () -> LTerm_ui.quit ui

let draw ui matrix coord =
  let size = LTerm_ui.size ui in
  let ctx = LTerm_draw.context matrix size in
  LTerm_draw.clear ctx;
 (*  LTerm_draw.draw_frame_labelled ctx { row1 = 0; col1 = 0; row2 = size.rows; col2 = size.cols } 
  ~alignment:H_align_center "Use arrow keys to move text" LTerm_draw.Light;
 *)  if size.rows > 2 && size.cols > 2 then begin
    let ctx = LTerm_draw.sub ctx { row1 = 1; col1 = 1; row2 = size.rows - 1; col2 = size.cols - 1 } in
    LTerm_draw.draw_styled ctx coord.row coord.col (eval [B_fg LTerm_style.lblue; S"Move me"; E_fg])
  end

let new_gui coord =  
Lazy.force LTerm.stdout
  >>= fun term ->
  LTerm_ui.create term (fun matrix size -> draw matrix size !coord)


let main () =(* Coordinates of the message. *)
  let coord = ref { row = 0; col = 0 } in
  new_gui coord
  >>= fun ui ->
  Lwt.finalize (fun () -> get_inputs ui coord) (quit_gui ui)

let () = Lwt_main.run (main ())