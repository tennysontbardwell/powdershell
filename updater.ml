...
| ((ElemAdd i)::t) -> ArrayModel.set_pixel i.loc (if i.elem = "erase" then None else Some {name=i.elem}) g |> ignore; receive_input t g
| _ -> g
...
| Move_exc (name, start, final, _) ->
  if ArrayModel.in_grid grid final then
    ...
    | (Some {name=a_name}, Some {name=b_name}) when a_name=name ->
        (List.assoc a_name rules).density > (List.assoc b_name rules).density
    | _ -> false
  else false
